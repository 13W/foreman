...
import { entriesToPlan, EntriesToPlanError } from './entries-to-plan.js';
...
describe('entriesToPlan', () => {
  ...
  it('missing _meta.assignedAgent falls back to first worker with warning', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      ...
      expect.stringContaining('missing _meta.assignedAgent'),
    );
  });

  it('unknown assignedAgent falls back to first worker with warning', () => {
    const warnSpy = vi.spyOn(logger, 'warn');
    const entries = [
    ...
    expect(plan.batches[0].subtasks[0].assigned_agent).toBe('worker_a');
    expect(warnSpy).toHaveBeenCalledWith(
      expect.objectContaining({ assignedAgent: 'nonexistent_worker' }),
